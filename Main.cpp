...
void averageF(FILE* file, char* nameOfFile) {// крч, можно не передовая указатель на файл, просто везде fopen
	file = fopen(nameOfFile, "r");
...
void swap(FILE* file,char* nameOfFile) {
	...
	fpos_t begin = 0, end;
	//rewind(file);
	//while (1) {                  //Пропуск пробелов в начале
	//	fscanf(file, "%c", &symb);
	//	if (symb == ' ') {
	//		fseek(file, -1, 1);
	//		fgetpos(file, &begin);
...
void revers(FILE* file, fpos_t begin, fpos_t end) {
	if (!file) return;
	char changSymb, symbBeg, symbEnd;
	while(1) {
		rewind(file);
		fseek(file, begin, 0);
		...
		fseek(file, -1, 1);
		fprintf(file, "%c", &symbBeg);
		fseek(file, begin, 0);
		fprintf(file, "%c", &symbEnd);
		begin++;
		end--;
		if (begin >= end) break;
	}
